...
def test_crimson_objectstore(ctx, config, cli_remote, REP_POOL, REP_NAME):
    ...
    # Test --op list-pgs operation
    log.info("Test --op list-pgs functionality")
    ...
    for remote in osds.remotes.keys():
        for role in osds.remotes[remote]:
            ...
            cmd = (prefix + "--op list-pgs").format(id=osdid)
            try:
                ...
                log.warning("--op list-pgs failed with status {ret} for osd.{id}".
                          format(ret=e.exitstatus, id=osdid))

    # Test --op list-objects operation  
    log.info("Test --op list-objects functionality")
    for remote in osds.remotes.keys():
        for role in osds.remotes[remote]:
            if not role.startswith("osd."):
                continue
            ...
            for pg in pgs[osdid]:
                cmd = (prefix + "--op list-objects {pg}").format(id=osdid, pg=pg)
                try:
                    ...
                    log.warning("--op list-objects failed with status {ret} for pg {pg}".
    ...
    for fmt in formats:
        for remote in osds.remotes.keys():
            for role in osds.remotes[remote]:
                ...
                cmd = (prefix + "--format {fmt} --op list-pgs").format(id=osdid, fmt=fmt)
                try:
                    result = remote.sh(cmd, check_status=False)
    ...
    invalid_cases = [
        ("--data-path /nonexistent --store-type seastore --op list-pgs", 
         "nonexistent data path"),
        ("--data-path /tmp --store-type invalid --op list-pgs", 
         "invalid store type"),
        ("--store-type seastore --op list-pgs", 
         "missing data path"),
        ("--data-path /tmp --store-type seastore", 
         "missing operation"),
    ]
    ...
    if pgswithobjects and objsinpg:
        for remote in osds.remotes.keys():
            for role in osds.remotes[remote]:
                ...
                # Test list-omap
                log.info("Testing --op list-omap")
                cmd = (prefix + "--op list-omap {pg} {obj}").format(
                ...
                try:
                    ...
                    log.warning("--op list-omap failed with status {ret}".format(ret=e.exitstatus))

                # Create a temporary file for set-omap test using a safer path
                test_key = "test_omap_key"
                test_value = "test_omap_value"
                # Use a path that should be writable on the remote system
                temp_file = os.path.join(TEUTHDIR, "test_omap_value.txt")
                remote.write_file(temp_file, test_value)

                # Test set-omap with in keyword
                log.info("Testing --op set-omap with in keyword")
                cmd = (prefix + "--op set-omap {pg} {obj} {key} in {file}").format(
                    id=osdid, pg=test_pg, obj=test_object, key=test_key, file=temp_file)
                try:
                    result = remote.sh(cmd, check_status=False)
                    log.info("set-omap completed for key {key}".format(key=test_key))
                except CommandFailedError as e:
                    log.warning("--op set-omap failed with status {ret}".format(ret=e.exitstatus))

                # Test get-omap (try to get the key we just set)
                log.info("Testing --op get-omap")
                cmd = (prefix + "--op get-omap {pg} {obj} {key}").format(
                ...
                try:
                    ...
                    log.warning("--op get-omap failed with status {ret}".format(ret=e.exitstatus))

                # Test get-omap with out keyword
                log.info("Testing --op get-omap with out keyword")
                output_file = os.path.join(TEUTHDIR, "test_omap_output.txt")
                cmd = (prefix + "--op get-omap {pg} {obj} {key} out {file}").format(
                    id=osdid, pg=test_pg, obj=test_object, key=test_key, file=output_file)
                try:
                    result = remote.sh(cmd, check_status=False)
                    # Read the output file to verify
                    output_content = remote.read_file(output_file)
                    if output_content.strip() == test_value:
                        log.info("get-omap with out keyword PASSED")
                    else:
                        log.warning("get-omap with out keyword failed: expected {exp}, got {actual}".format(
                            exp=test_value, actual=output_content.strip()))
                        ERRORS += 1
                except CommandFailedError as e:
                    log.warning("--op get-omap with out keyword failed with status {ret}".format(ret=e.exitstatus))

                # Test remove-omap
                log.info("Testing --op remove-omap")
                cmd = (prefix + "--op remove-omap {pg} {obj} {key}").format(
                    id=osdid, pg=test_pg, obj=test_object, key=test_key)
                try:
                    result = remote.sh(cmd, check_status=False)
                    log.info("remove-omap completed for key {key}".format(key=test_key))
                except CommandFailedError as e:
                    log.warning("--op remove-omap failed with status {ret}".format(ret=e.exitstatus))
                    ERRORS += 1

                # Test get-omap after remove (should be empty)
                log.info("Testing --op get-omap after remove-omap (should be empty)")
                cmd = (prefix + "--op get-omap {pg} {obj} {key}").format(
                    id=osdid, pg=test_pg, obj=test_object, key=test_key)
                try:
                    omap_value = remote.sh(cmd, check_status=False)
                    ...
                    log.warning("--op get-omap after remove-omap failed with status {ret}".format(ret=e.exitstatus))
                    ERRORS += 1

                # Test omap argument validation
                log.info("Testing OMAP argument validation")
                omap_invalid_cases = [
                    ("--op list-omap {pg}".format(pg=test_pg), "list-omap without object"),
                    ("--op get-omap {pg} {obj}".format(pg=test_pg, obj=test_object), 
                     "get-omap without omap-key"),
                    ("--op set-omap {pg} {obj} {key}".format(
                        pg=test_pg, obj=test_object, key=test_key), "set-omap without in file"),
                ]
                ...
                try:
                    remote.run(args=['rm', '-f', temp_file, output_file])
                except:
                    pass
