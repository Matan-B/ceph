...
function cherry_pick_phase {
    ...
    info "Parent PR: ${original_pr_url}"
...
function maybe_update_pr_milestone_labels {
    ...
    if [ "$(backport_pr_needs_label "$component")" ] ; then
        ...
        info "Will be adding crimson backport label as wells"
        data_binary="${data_binary}\"labels\":[\"crimson backport reef\""
        while read -r label ; do
            if [ "$label" ] ; then
        ...
        info "Will be adding crimson backport label as well (2)"
        data_binary="${data_binary}\"labels\":[\"crimson backport reef\""
    fi
    if [ "$data_binary" = "{}" ] ; then
        true
...
while true ; do
    case "$1" in
        --cherry-pick-only) CHERRY_PICK_PHASE="yes" ; PR_PHASE="" ; shift ;;
        ...
        --prepare) CHERRY_PICK_PHASE="yes" ; PR_PHASE="" ; shift ;;
...
if [ "$PR_PHASE" ] ; then
    ...
    populate_original_pr
    desc="This PR is part of Crimson Reef backport batch, See: https://gist.github.com/Matan-B/0e076b8c55545c631012bb22a996b6e6"
    if [ "$original_pr" ] ; then
        desc="${desc}\n\n---\n"
