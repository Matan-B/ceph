...
// a counterpart of the `make_writeable()` in the classical OSD.
// the main difference is this one executes before, not after,
// all other operations. the rationale behind the departure is
// simplicity (no new / old obs, no txn reordering).
//
...
void OpsExecuter::commit_clone_n_snapset(
  std::vector<pg_log_entry_t>& log_entries)
{
  assert(!txn.empty());
  if (cloning_ctx) {
