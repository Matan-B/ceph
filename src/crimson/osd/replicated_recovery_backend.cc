  ...
  pg_shard_t pg_shard)
{
  ...
  if (soid.snap && soid.snap < CEPH_NOSNAP) {
    ...
    logger().debug("push_to_replica snapset is {}",
                   ssc->snapset);
    assert(pg.get_shard_missing().contains(pg_shard));
    //push_op->recovery_info.ss = ssc->snapset;
    //calc_clone_subsets(data_subset, clone_subsets);
    ...
    logger().debug("push_to_replica snapset is {}",
                   ssc->snapset);
    //calc_head_subsets(data_subset, clone_subsets);
  }
  return prep_push(soid,
                   need,
                   pg_shard,
  ...
  map<hobject_t, interval_set<uint64_t>>& clone_subsets)
{
  ...
  //this is the calc_head it will be seperated:
