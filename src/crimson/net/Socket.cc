...
struct bufferlist_consumer {
  ...
  seastar::future<consumption_result_type> operator()(tmp_buf&& data) {
    if (remaining >= data.size()) {
      ...
      remaining -= data.size();
      bl.append(buffer::create_delay_release(std::move(data)));
      if (remaining > 0) {
    ...
    if (remaining > 0) {
      // consume the front
      bl.append(buffer::create_delay_release(data.share(0, remaining)));
...
Socket::read_exactly(size_t bytes) {
  ...
  return try_trap_pre(next_trap_read).then([bytes, this] {
    ...
    return in.read_exactly(bytes).then([bytes](auto buf) {
      bufferptr ptr(buffer::create_delay_release(buf.share()));
      if (ptr.length() < bytes) {
        throw std::system_error(make_error_code(error::read_eof));
      }
      inject_failure();
