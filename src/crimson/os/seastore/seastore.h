...
enum class txn_stage_t : uint8_t {
    ...
    // Sub-phases of submit_transaction. These together make up the bulk of
    // SUBMIT_TOTAL but don't sum to it exactly (minor gaps like the pipeline
    // complete() stage are unaccounted). All are within the collection-lock hold
    // EXCEPT SUBMIT_JOURNAL (post-lock), so:
    //   collection-lock hold ~= BUILD + every SUBMIT_* below except SUBMIT_JOURNAL.
